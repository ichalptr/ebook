 ...
 * Tag kategori (pojok kiri atas cover) otomatis muncul kalau
 * category_name tersedia di baris $book — ini konsisten ada di semua
 * query pemanggil (index.php, katalog.php, rak_saya.php).
 */
function render_book_card(array $book, string $wrapClass = 'col-6 col-md-3', bool $showFoldIcon = false): void {
    ...
    $category = trim($book['category_name'] ?? '');
    ?>
        ...
        <a href="<?= BASE_URL ?>/detail.php?id=<?= (int)$book['id'] ?>" class="text-decoration-none text-dark">
            <div class="card book-card reveal">
                <div class="book-cover-wrap">
                    ...
                    <?php if ($category): ?>
                        <span class="cat-tag"><?= htmlspecialchars($category) ?></span>
                    <?php endif; ?>
                    <?php if ($showFoldIcon): ?>
