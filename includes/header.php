...
<nav class="navbar navbar-expand-lg pl-navbar">
  <div class="container">
    ...
    <button class="navbar-toggler border-0 text-white" type="button" data-bs-toggle="collapse" data-bs-target="#navMain" aria-label="Buka menu" style="filter:invert(1);">
    ...
    <div class="collapse navbar-collapse" id="navMain">
      ...
      <form class="d-flex me-3 desktop-search" role="search" action="<?= BASE_URL ?>/katalog.php" method="get">
        <input class="form-control form-control-sm" type="search" name="q" placeholder="Cari judul/penulis..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>" style="min-width:230px;">
      </form>
        ...
        <?php if ($user): ?>
          ...
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle d-flex align-items-center gap-2" href="#" data-bs-toggle="dropdown">
              <span class="nav-avatar"><?= htmlspecialchars(mb_strtoupper(mb_substr($user['name'], 0, 1))) ?></span>
              <?= htmlspecialchars($user['name']) ?>
            </a>
            <ul class="dropdown-menu dropdown-menu-end">
              <li><a class="dropdown-item" href="<?= BASE_URL ?>/rak_saya.php"><i class="bi bi-bookmark me-2"></i>Rak Saya</a></li>
