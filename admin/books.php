...
<div class="admin-card">
  <div class="admin-card-pad pb-0">
    <form method="get" class="d-flex flex-wrap gap-2 align-items-center mb-3">
      ...
      <?php if ($q): ?><a href="<?= BASE_URL ?>/admin/books.php" class="btn btn-link text-decoration-none">Reset</a><?php endif; ?>
      <a href="<?= BASE_URL ?>/admin/book_form.php" class="btn btn-forest ms-auto"><i class="bi bi-plus-circle"></i> Tambah Buku</a>
    </form>
    <div class="small text-muted mb-2"><?= count($books) ?> buku ditemukan<?= $q ? ' untuk "' . htmlspecialchars($q) . '"' : '' ?></div>
  </div>
